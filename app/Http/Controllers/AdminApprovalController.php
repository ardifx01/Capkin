...
{
    /**
     * Menampilkan daftar dokumen yang menunggu persetujuan Admin Approval.
     */
    public function view_pending_data(Request $request)
    {
        ...
        // Inisialisasi query
        $dataIkuQuery = DataIku::whereIn('status', ['pending_binagram', 'pending_approval'])
    ...
    public function view_approved_master_data(Request $request)
    {
        ...
        return view('adminapproval.approved-master-data-ap', compact('dataIku'));
    }
    ...
    public function view_rejected_master_data(Request $request)
    {
        ...
        return view('adminapproval.rejected-master-data-ap', compact('dataIku'));
    }
    ...
    public function approve_data(Request $request, $id)
    {
        try {
            ...
            // Pastikan status sebelumnya adalah pending_approval
            if ($dataIku->status === 'pending_approval') {
                $dataIku->update([
                    'status' => 'pending_binagram',
                    'approve_by_approval' => Auth::id() // Menyimpan ID Approval
                ]);

                return response()->json([
                    'success' => [
                        "title" => "Data Approved Successfully",
                        "message" => "Data berhasil disetujui oleh Approval."
                    ]
                ], 200);
            }
            ...
            return response()->json([
                'error' => [
                    ...
                    "message" => "Data tidak berada dalam status yang dapat disetujui oleh Approval."
                ]
            ], 400);
        } catch (\Exception $e) {
            return response()->json([
    ...
    public function reject_data(Request $request, $id)
    {
        ...
        // Cek apakah data sudah disetujui oleh approval
        if ($dataIku->status === 'pending_approval') {
            $dataIku->status = 'rejected_approval';
            $dataIku->reject_comment = $request->reject_comment; // Simpan komentar penolakan
            $dataIku->reject_by_approval = Auth::id();
            $dataIku->save();
